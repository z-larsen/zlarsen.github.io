...
<figure>
<img src="https://learn.microsoft.com/en-us/azure/dns/media/dns-resolver-overview/resolver-architecture.png" alt="Azure DNS Private Resolver architecture showing inbound and outbound endpoints for hybrid DNS resolution between on-premises and Azure">
<figcaption>Azure DNS Private Resolver architecture: inbound endpoints accept queries from on-premises, outbound endpoints forward queries to external DNS servers via forwarding rulesets. Source: Microsoft Learn</figcaption>
</figure>
...
DNS Security Policies can be configured through the **Azure portal** or **Azure PowerShell**. The PowerShell cmdlets ship in the `Az.DnsResolver` module, and the resources use the `DnsResolverPolicy` naming convention.

**Example PowerShell Commands:**

```powershell
# Create a DNS security policy
$policy = New-AzDnsResolverPolicy -Name "MySecurityPolicy" -ResourceGroupName "MyRG" -Location "EastUS"

# Create a domain list (note the trailing dots on FQDNs)
$domainList = New-AzDnsResolverDomainList -Name "BlockList" -ResourceGroupName "MyRG" -Location "EastUS" -Domain @("malicious.example.com.","bad.domain.")

# Create a DNS security rule that blocks the domain list
New-AzDnsResolverPolicyDnsSecurityRule -Name "BlockRule" -ResourceGroupName "MyRG" -DnsResolverPolicyName "MySecurityPolicy" -Location "EastUS" -Priority 100 -ActionType "Block" -ActionBlockResponseCode "SERVFAIL" -DnsResolverDomainList @{ id = $domainList.Id } -DnsSecurityRuleState "Enabled"

# Link the policy to a VNet
New-AzDnsResolverPolicyVirtualNetworkLink -Name "VNetLink" -ResourceGroupName "MyRG" -DnsResolverPolicyName "MySecurityPolicy" -Location "EastUS" -VirtualNetworkId "/subscriptions/.../vnet"
```
